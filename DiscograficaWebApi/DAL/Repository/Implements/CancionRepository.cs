...
public class CancionRepository : Repository<Cancion>, ICancionRepository
{
    ...
    {
        var result = await _context.Canciones.Where(c => c.Duracion == duracion).ToListAsync();
    ...
    {
        var result = await _context.Canciones.Where(c => c.GeneroMusical == genero).ToListAsync();
    ...
    {
        var result = await _context.Canciones.Where(c => c.Artista == artista).ToListAsync();
